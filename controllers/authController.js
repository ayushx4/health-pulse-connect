...
async function verifyOtpAndCreateGuardian(req, res) {
  try {
    ...
    const createdAccount = await createAccount(
      GuardianModel,
      verifiedOtpRecord.email,
      verifiedOtpRecord.password
    );
    const accessToken = await createdAccount.generateAccessToken();

    return res
      .status(200)
      .json({ accessToken, message: 'Guardian account created successfully' });
  } catch (error) {
...
async function guardianLogin(req, res) {
  try {
    ...
    if (!guardian) throw new Error('No account found with this email');
